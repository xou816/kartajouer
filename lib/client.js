...
game.ClientCard.prototype.draw_upside = function(ctx) {
    ...
    if (this.value <= 56) {
        ...
        ctx.font = 0.8*size+'px sans-serif';
...
game.ClientSpot.prototype.transfer_drag_stack = function(other_spot) {
    game.Spot.prototype.transfer_drag_stack.call(this, other_spot);
...
game.ClientBoard = function(dom_node, socket, name) {
    ...
    this.zoom_val = 1;
    this.magnify = new Transform();
    this.magnify.scale(this.zoom_val, this.zoom_val);
    this.back_magnifier = new game.CanvasObject();
    this.back_magnifier.parent = this.background;
    this.back_magnifier.matrix = this.magnify;
    this.fore_magnifier = new game.CanvasObject();
    this.fore_magnifier.parent = this.foreground;
    this.fore_magnifier.matrix = this.magnify;
    ...
    this.node.addEventListener('DOMMouseScroll', (function(event) { // Firefox
...
game.ClientBoard.prototype.update_board = function(data) {
    ...
    data.board.forEach(function(spot_data) { // Spots pushed in order to keep indices and ids being the same
        ...
        spot.parent = this.back_magnifier;
...
game.ClientBoard.prototype.update_spot = function(spot_data) {
    ...
    spot.update_layout();
    spot_data.stack.forEach(function(card_data) {
        this.cards[card_data.id].value = card_data.value;
    }, this);
    spot.update_visibility();
    this.draw();
};
...
game.ClientBoard.prototype.zoom = function(delta) {
    this.allow_pan = true;
    var tmp = new Transform();
    tmp.scale(this.zoom_val, this.zoom_val);
    tmp.invert();
    tmp.multiply(this.magnify);
    this.magnify = tmp;
    if (delta < 0 && this.zoom_val < 1.5) {
        this.zoom_val += 0.1;
    } else if (delta > 0 && this.zoom_val > 0.75) {
        this.zoom_val -= 0.1;
    }
    this.clear();
    this.magnify.scale(this.zoom_val, this.zoom_val);
...
game.ClientBoard.prototype.do_drag = function(spot, matches, draw_helpers) {
    ...
    spot.drag_stack.parent = this.fore_magnifier;
    spot.drag_stack.draw();
...
game.ClientBoard.prototype.canvas_drag_handler = function(x, y) {
    if (this.mouse_spot === null) {
        ...
        if (this.mouse_spot === null) { // No drag initiated
            this.pan_origin = this.background.convert_coords(x, y);
        }
    }
};
...
game.ClientBoard.prototype.canvas_move_handler = function(x, y) {
    if (this.mouse_spot !== null) {
        ...
        if (this.pan_origin !== null && this.allow_pan) {
            var x0 = this.pan_origin[0], y0 = this.pan_origin[1];
            var conv = this.background.convert_coords(x, y);
            var x1 = conv[0], y1 = conv[1];
            this.clear();
            this.magnify.translate(x1-x0, y1-y0);
            this.draw();
            this.pan_origin = [x1, y1];
        }
        this.spots.forEach(function(spot) {
...
game.ClientBoard.prototype.do_drop = function(from, to, clear_helpers) {
    ...
    this.clear();
    from.drag_stack.parent = this.back_magnifier;
    to.stack.clear();
