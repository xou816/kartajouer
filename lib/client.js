...
game.Canvas.prototype.autoscale = function(factor) {
    var w = this.node.parentNode.offsetWidth, h = this.node.parentNode.offsetHeight;
    this.node.width = w;
    this.node.height = h;
    var tr_x = w > h ? 1 : 0;
    var tr_y = h > w ? 1 : 0;
    var factor = (typeof factor === 'undefined' ? 1 : factor)*Math.min(w, h)/100;
    this.matrix.reset();
...
game.ClientBoard.prototype.resize = function() {
    this.foreground.autoscale(this.zoom);
    this.foreground.rotate(this.orientation);
    this.background.autoscale(this.zoom);
