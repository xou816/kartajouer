...
game.Layout.upside = function(spot, stack, focus) {
    stack.forEach(function(card) {
        card.up = true;
    });
};

game.Layout.downside = function(spot, stack, focus) {
    stack.forEach(function(card) {
        card.up = false;
    });
};
...
game.ClientSpot.prototype.set_layout = function(type, focus) {
    var stack = type === 'drop' ? this.stack : this.drag_stack;
    this.layout[this.layout_context()][type].split(',').forEach(function(name) {
        var fun = game.Layout[name.trim()];
        fun(this, stack, focus);
    }, this);
};
    ...
    if (this.mouse_spot !== null) {
        ...
        this.spots.forEach(function(spot) {
            spot.stack.some(function(card) {
                ...
                if (matches.length > 0 && spot.has_layout('drop', 'upside')) {
