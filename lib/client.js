...
game.ClientBoard = function(dom_node, socket, name) {
    ...
    // Client events
    this.node.addEventListener('mousewheel', this.zoom.bind(this));
...
game.ClientBoard.prototype.zoom = function(event) {
    this.allow_pan = true;
    ...
    this.magnify = tmp;
    if (event.deltaY < 0 && this.zoom_val < 1.5) {
        this.zoom_val += 0.1;
    } else if (event.deltaY > 0 && this.zoom_val > 0.75) {
        this.zoom_val -= 0.1;
    }
    this.clear();
    this.magnify.scale(this.zoom_val, this.zoom_val);
